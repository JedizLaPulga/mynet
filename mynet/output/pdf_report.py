...
class PDFReportGenerator:
    ...
    def _build_title_page(self, results: Dict[str, Any], kwargs: dict) -> list:
        ...
        # Title
        elements.append(Paragraph("Security Scan Report", self.styles['Title']))

        # Target info
        targets = list(results.keys())
        if targets:
            target_str = targets[0] if len(targets) == 1 else f"{len(targets)} targets"
            elements.append(Paragraph(f"Target: {target_str}", self.styles['Subtitle']))

        # Date
        report_date = datetime.now().strftime("%B %d, %Y at %H:%M")
        elements.append(Paragraph(f"Generated: {report_date}", self.styles['Subtitle']))
        ...
        # Tool branding
        elements.append(Paragraph("Generated by MyNet Scanner", self.styles['Subtitle']))
    ...
    def _build_findings_summary(self, results: Dict[str, Any]) -> list:
        ...
        elements.append(Paragraph("Findings Summary", self.styles['SectionHeader']))
        ...
        if medium_findings:
            elements.append(Paragraph(
                f"🟠 Medium Severity ({len(medium_findings)})",
                self.styles['SubSection']
            ))
    ...
    def _build_detailed_findings(self, results: Dict[str, Any]) -> list:
        ...
        for host, data in results.items():
            elements.append(Paragraph(f"Target: {host}", self.styles['SubSection']))
